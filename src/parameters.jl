...
macro build(name, N, with_basis)
    ...
    if with_basis
        return quote
            function $(esc(name))(b_def, arg::$T1) where {K<:Label{$N, <:Integer}, V, N}
                $(Expr(:call, Expr(:curly, esc(:new), :K, :V), Expr(:call, Dict, :arg), :b_def))
            end

            function $(esc(name))(b_def, arg::$T1) where {K<:NTuple{$N, <:Integer}, V, N}
                $(Expr(:call, esc(name), :b_def, Expr(:(...), Expr(:call, :map, esc(t2l), :arg))))
            end
        end
    else
        return quote
            function $(esc(name))(arg::$T1) where {K<:Label{$N, <:Integer}, V, N}
                $(Expr(:call, Expr(:curly, esc(:new), :K, :V), Expr(:call, Dict, :arg)))
            end

            function $(esc(name))(arg::$T1) where {K<:NTuple{$N, <:Integer}, V, N}
...
function _multi_line(io, x::T) where T<:NewParams
    i = length(keytype(x._vals).types[1].types) ≡ 1 ? 1 : Base.:(:)
    indent = repeat(" ", get(io, :indent, 0)+2)
    v_string = join(["$(indent)$(k[i]) => $v" for (k, v) in x._vals], "\n")
    # Make convert "()" to "(All)" for to make GlobalParams more readable 
    v_string = replace(v_string, "()" => "All") 
...
function Base.show(io::O, x::T) where {T<:NewParams, O<:IO}
    ...
    if !get(io, :compact, false) && !haskey(io.dict, :SHOWN_SET)
        print(io, _multi_line(io, x))
    ...
    # representation is needed.
        ...
        targets = length(x) != 1 ? [[1, lastindex(keys_s)]] : [1:1]
        # Build the key list and print the message out
        k_string = join([k[s] for k in keys_s[targets...]], " … ")
        # Make convert "()" to "(All)" for to make GlobalParams more readable 
        k_string = replace(k_string, "()" => "All")
        indent = repeat(" ", get(io, :indent, 0))
        print(io, "$(indent)$(nameof(T))($(k_string))")
    end
end
...
# Todo
 - check that r0 is still relevant
"""
...
# Todo:
- add λₙ & λₗ as parameters
- generate constructor to allow for arbitrary fields
"""
struct OffSiteParaSet <: ParaSet
    ...
    function OffSiteParaSet(ν::T₁, deg::T₂, b_cut::T₃, e_cut_out::T₄
        ) where {T₁<:NewParams, T₂<:NewParams, T₃<:NewParams, T₄<:NewParams}
        ν::NewParams{<:Label, <:Integer}
        deg::NewParams{<:Label, <:Integer}
        b_cut::NewParams{<:Label, <:AbstractFloat}
        e_cut_out::NewParams{<:Label, <:AbstractFloat}
        new(ν, deg, b_cut, e_cut_out)
    end
...
function Base.show(io::O, x::T) where {T<:ParaSet, O<:IO}
    print(io, "$(nameof(T))")
    if !get(io, :compact, false) && !haskey(io.dict, :SHOWN_SET)
        new_io = IOContext(io.io, :indent=>get(io, :indent, 0)+4, :compact=>get(io, :compact, false))
        for f in fieldnames(T)
            print(new_io, join(["\n", repeat(" ", get(io, :indent, 0)+2)]), "$f: ", getfield(x, f))
        end
