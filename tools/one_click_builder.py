...
def parse_system(source: str, destination: Group):
    ...
    H_gamma = data.create_dataset('H_gamma', data=H_matrix.sum(axis=0))
    set_unit(H_gamma, 'Ha')
    # data.create_dataset('S_gamma', data=read_overlap_matrix(join(source, 'overlap-matrix.out')).T)
    data.create_dataset('S_gamma', data=S_matrix.sum(axis=0))
...
def extract_dataset(parent_directory: str):

    parent_directory = os.path.normpath(parent_directory)
    database_name = os.path.join(parent_directory, os.path.basename(parent_directory)+'.h5')
    subfolders = list_subfolders(parent_directory)

    parent_directory_length = len(parent_directory.split(os.sep))

    with File(database_name, 'w') as db:
        for directory in subfolders:
            if os.path.exists(os.path.join(directory, 'basis-indices.out')):
                try:
                    group_name = os.path.join(*directory.split(os.sep)[parent_directory_length:])
                    parse_system(directory, db.create_group(group_name))
                except Exception as e:
                    print(directory)
                    raise e from e


def set_sparse_data(destination: Group, data: Array):
    # data = csc_matrix(data)
    # destination.create_dataset('data', data=data.data)
    # destination.create_dataset('indices', data=data.indices)
    # destination.create_dataset('indptr', data=data.indptr)
    # destination.create_dataset('shape', data=data.shape)
    if data.ndim == 2:
        data = csc_matrix(data)
        destination.create_dataset('data', data=data.data)
        destination.create_dataset('indices', data=data.indices)
        ...
        data_list = [csc_matrix(data_ele) for data_ele in data]
        destination.create_dataset('data', data=np.hstack([data.data for data in data_list]))
        destination.create_dataset('indices', data=np.hstack([data.indices for data in data_list]))
        destination.create_dataset('sizes', data=[data.data.size for data in data_list])
        destination.create_dataset('indptr', data=np.hstack([data.indptr for data in data_list]))
        destination.create_dataset('shape', data=[data.shape for data in data_list])
    else:
        raise ValueError("the dimension of the data should either be 2 or 3")


def parse_system_sparse(source: str, destination: Group):
    ...
    H_matrix = matrix_builder.regenerate(open(join(source, 'rs_hamiltonian.out')))
    # H_matrix = np.transpose(H_matrix, (0, 2, 1)) 
    # we are dealing with sparse matrix, so we don't need to transpose the matrix 
    # H = data.create_dataset('H', data=H_matrix)
    H = data.create_group('H')
    set_sparse_data(H, H_matrix)
    set_unit(H, 'Ha')
    S_matrix = matrix_builder.regenerate(open(join(source, 'rs_overlap.out')))
    # S_matrix = np.transpose(S_matrix, (0, 2, 1))
    # data.create_dataset('S', data=S_matrix)
    S = data.create_group('S')
    set_sparse_data(S, S_matrix)
    # H_gamma = data.create_dataset('H_gamma', data=read_hamiltonian_matrix(join(source, 'hamiltonian.out')).T)
    # H_gamma = data.create_dataset('H_gamma', data=H_matrix.sum(axis=0))
    H_gamma = data.create_group('H_gamma')
    set_sparse_data(H_gamma, data=H_matrix.sum(axis=0))
    set_unit(H_gamma, 'Ha')
    # data.create_dataset('S_gamma', data=read_overlap_matrix(join(source, 'overlap-matrix.out')).T)
    # data.create_dataset('S_gamma', data=S_matrix.sum(axis=0))
    S_gamma = data.create_group('S_gamma')
    set_sparse_data(S_gamma, data=S_matrix.sum(axis=0))


def extract_dataset_sparse(parent_directory: str):

    parent_directory = os.path.normpath(parent_directory)
    database_name = os.path.join(parent_directory, os.path.basename(parent_directory)+'_sparse.h5')
    subfolders = list_subfolders(parent_directory)
    ...
    with File(database_name, 'w') as db:
        for directory in subfolders:
            if os.path.exists(os.path.join(directory, 'basis-indices.out')):
                try:
                    group_name = os.path.join(*directory.split(os.sep)[parent_directory_length:])
                    parse_system_sparse(directory, db.create_group(group_name))
...
# def extract_dataset_slim(parent_directory: str):

#     ###discard H for reference data

#     parent_directory = os.path.normpath(parent_directory)
#     database_name = os.path.join(parent_directory, os.path.basename(parent_directory)+'_slim.h5')
#     subfolders = list_subfolders(parent_directory)
...
#                     group_name = os.path.join(*directory.split(os.sep)[parent_directory_length:])
#                     parse_system(directory, db.create_group(group_name))
#                     # if 'reference_data' in group_name:
#                     #     HMM = db[os.path.join(group_name, "Data/H")][:].sum(axis=0)
#                     #     del(db[os.path.join(group_name, "Data/H")])
#                     #     del(db[os.path.join(group_name, "Data/H_gamma")])
#                     #     db.create_dataset(os.path.join(group_name, "Data/H"), data=HMM)
#                     #     SMM = db[os.path.join(group_name, "Data/S")][:].sum(axis=0)
#                     #     del(db[os.path.join(group_name, "Data/S")])
#                     #     del(db[os.path.join(group_name, "Data/S_gamma")])
#                     #     db.create_dataset(os.path.join(group_name, "Data/S"), data=SMM)                    if 'reference_data' in group_name:
#                     HMM = db[os.path.join(group_name, "Data/H")][:].sum(axis=0)
#                     del(db[os.path.join(group_name, "Data/H")])
#                     del(db[os.path.join(group_name, "Data/H_gamma")])
#                     db.create_dataset(os.path.join(group_name, "Data/H"), data=HMM)
...
#                     del(db[os.path.join(group_name, "Data/S")])
#                     del(db[os.path.join(group_name, "Data/S_gamma")])
#                     db.create_dataset(os.path.join(group_name, "Data/S"), data=SMM)
...
if __name__ == "__main__":
    target_directory = "/gpfs/home/chem/mssrdn/FHI-aims/Dataset/data_to_be_collected_split_dos/Paper"
    # extract_dataset(parent_directory=target_directory)
    # extract_dataset_slim(parent_directory=target_directory)
    extract_dataset_sparse(parent_directory=target_directory)



file_sparse = File("./Paper/Paper_sparse.h5")