...
@add_arg_table args begin
    ...
    "--index", "-i"
        help = "Atoms from each file to include. Default is 1:729, i.e. all atoms"
        arg_type = Vector{Int64}
        default = Vector(1:2:729)
        eval_arg = true
    "--indextest", "-t"
        help = "Atoms from each file to include. Default is 1:729 i.e. all atoms"
        arg_type = Vector{Int64}
        default = Vector(2:2:729)
        eval_arg = true
    "--rcut", "-r"
...
function expand_arg(key::String, value::AbstractVector)
    if length(value) == 1
        if key == "degree"
            return repeat([value[1], 4], 1, 6)
        else    
            return repeat(value, 2, 6)
        end
    elseif length(value) == 2
        return repeat(value, 1, 6)
    elseif length(value) == 12
        return reshape(value, 2, 6)
    else
...
data = Data(parsed_args["name"], parsed_args["index"])
# @show data
