...
database_path = "./Data/Datasets/dyn-wd-500K_3.h5"
data_name = split(basename(database_path), ".")[1]
output_path = joinpath("./Result/hyperparameter_searching_dm_r0", data_name) 
nsamples = 128
mkpath(output_path)
...
function get_model(basis_definition::BasisDef, d_max::Int64=14, r_cutoff::Float64=6., r₀::Float64=0.9,
    ...
    off_site_parameters = OffSiteParaSet(
        ...
        @info "finished buiding a model with d_max=$(d_max), r_cutoff=$(r_cutoff), r₀=$(r₀)"
...
function cross_validation(model::Model, database_path::String, nfolds::Int=5)
    ...
    errors = []
    atomsv = []
    for fold in nfolds:nfolds   # 1:nfolds
...
for r₀ in 0.9:0.3:3.1
    d_max = 14
    r_cutoff = 10.
    dict_name = (d_max, r₀)
    model = get_model(basis_definition, d_max, r_cutoff, r₀, model_type)
    data_dict_sub = cross_validation(model, database_path, nfolds)
    data_dict[dict_name] = data_dict_sub
    open(joinpath(output_path, "data_dict_d_max_14_r_cutoff.jls"), "w") do file
        serialize(file, data_dict)
    end
    plot_hyperparams(data_dict, "r_cut", output_path_figs)
    @info "finished testing a model with d_max=$(d_max), r_cutoff=$(r_cutoff), r₀=$(r₀)"
end